...
#define PROID		"smarthome_tt"

#define AUTH_INFO	"smarthome_tt"

#define DEVID		"smarthome_tt"
	...
	switch(type)
	{
		...
		case MQTT_PKT_PUBLISH:														//接收的Publish消息
			...
			if(result == 0)
			{
				...
				if (!json)UsartPrintf(USART1,"Error before: [%s]\n",cJSON_GetErrorPtr());
				else
				{

					json_value = cJSON_GetObjectItem(json , "target");
					DEBUG_LOG("json_value: [%s]\n",json_value->string);
					...
					else if(strstr(json_value->valuestring,"OPEN") != NULL)
					{
						json_value = cJSON_GetObjectItem(json , "value");
						if(json_value->valueint)	
						{
					...
					else if(strstr(json_value->valuestring,"INCREASE") != NULL)
					{
						json_value = cJSON_GetObjectItem(json,"value");
						if(json_value->valueint) 	lightva = lightva + 20;
						else lightva = lightva - 20;
					}
//					else
//					{
//						json_value = cJSON_GetObjectItem(json , "value");
